<?php
	...
	public function addcompany($data){ 
		$approved= 1;
		$adminapproved = $this->config->get('jobsetting_company_register');
		if(!empty($adminapproved)) {
			$approved = 1;
		} else {
			$approved =0;
		}
		$this->db->query("INSERT INTO " .DB_PREFIX . "company SET approved='".(int)$approved."',country_id='".(int)$data['country_id']."',zone_id='".(int)$data['zone_id']."',
	...
	public function editCompany($company_id,$data){ 	
 	...
 	public function addPaymentHistory($data){ 
 		$sql = "INSERT INTO " .DB_PREFIX . "payment_history SET company_id='".(int)$this->company->getId()."',date_added=now()"; 
		$query = $this->db->query($sql);
 	}
