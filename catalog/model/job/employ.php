...
class ModelJobEmploy extends Model {
	public function addfirsttable($data){
		$approved= 1;
		$adminapproved = $this->config->get('jobsetting_employ_register');
		if(!empty($adminapproved)) {
			$approved = 1;
		} else {
			$approved =0;
		}
		$sql="INSERT INTO " . DB_PREFIX . "employ set approved='".(int)$approved."',fullname='".$this->db->escape($data['fullname'])."',image='".$this->db->escape($data['image'])."',
