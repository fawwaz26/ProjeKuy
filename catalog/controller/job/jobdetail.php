...
if(empty($this->request->get['job_id'])){
 ...
 if(empty($jobcompanyinfos['job_id'])){
		...
		$zone_info = $this->model_localisation_zone->getZone($job_info['zone_id']);
	   ...
	   // print_r($similarsjobinfos);die();
	    ...
	    foreach ($similarsjobinfos as $similarsjobinfo) {
			...
			$zone_info = $this->model_localisation_zone->getZone($similarsjobinfo['zone_id']);
