	    ...
	    //print_r($filter_data);die();
         ...
         $companytotal=$this->model_job_company->getjobcompany($filter_data);
          ...
          $jobdetalis=$this->model_job_company->getCompanys($filter_data);
			...
			$data['results'] = sprintf($this->language->get('text_pagination'), ($companytotal) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($companytotal - $this->config->get('config_limit_admin'))) ? $companytotal : ((($page - 1) * $this->config->get('config_limit_admin')) + $filter_limit), $companytotal, ceil($companytotal / $filter_limit));
